...
export default function AttendancePage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const todayRecord = attendanceRecords.find((record) => record.userId === user.id && record.date === today)
    ...
    <Layout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Track your daily attendance and view attendance history.</p>
        </div>
        ...
        <Tabs defaultValue="attendance-sheet">
          <TabsList>
            ...
            {(user.role === "super_admin" ||
              user.role === "admin" ||
              user.role === "manager" ||
              user.role === "finance") && <TabsTrigger value="team-attendance">Team Attendance</TabsTrigger>}
          </TabsList>
          <TabsContent value="attendance-sheet" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>Your Attendance History</CardTitle>
                <CardDescription>View your attendance records for the past month</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
